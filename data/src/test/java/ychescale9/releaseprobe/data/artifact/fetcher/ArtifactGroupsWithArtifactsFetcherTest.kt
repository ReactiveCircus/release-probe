...
class ArtifactGroupsWithArtifactsFetcherTest {
    ...
    fun `should fetch all artifact groups first, and then artifacts for each group concurrently, maintaining order`() {
        ...
        every { service.fetchArtifactGroups() } returns Single.just(listOf(
                artifactGroup1, artifactGroup2, artifactGroup3
        ))

        every { service.fetchArtifactsByGroupId(artifactGroup1.groupId.replace(".", "/")) } returns Single.just(listOf(
                artifactA, artifactB
        )).delay(fetchArtifactsDuration1, TimeUnit.SECONDS, testScheduler)

        every { service.fetchArtifactsByGroupId(artifactGroup2.groupId.replace(".", "/")) } returns Single.just(listOf(
                artifactC, artifactD
        )).delay(fetchArtifactsDuration2, TimeUnit.SECONDS, testScheduler)

        every { service.fetchArtifactsByGroupId(artifactGroup3.groupId.replace(".", "/")) } returns Single.just(listOf(
                artifactE
        ...
        testObserver.assertComplete()
                .assertValue(listOf(
                        ArtifactGroupWithArtifactsEntity(ArtifactGroupEntity("androidx.arch.core"), listOf(
                                ArtifactEntity(
                                        "androidx.arch.core",
                                        "core-common",
                        ...
                        ArtifactGroupWithArtifactsEntity(ArtifactGroupEntity("androidx.test"), listOf(
                                ArtifactEntity(
                                        "androidx.test",
                                        "core",
                        ...
                        ArtifactGroupWithArtifactsEntity(ArtifactGroupEntity("com.google.firebase"), listOf(ArtifactEntity(
                                "com.google.firebase",
                                "firebase-analytics",
